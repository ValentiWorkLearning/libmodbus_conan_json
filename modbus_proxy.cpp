...
public:
    ModbusRequestsProxyImpl(const ModbusConfigData& modbusConfigData)
    {
        m_pModbusHandle.reset(modbus_new_rtu(
            modbusConfigData.serialDevicePath.c_str(), modbusConfigData.modbusBaudrate, 'N', 8, 1));

        if (!m_pModbusHandle)
        {
            spdlog::error(
                "[modbus] Can't create RTU instance for {} interface",
                modbusConfigData.serialDevicePath);
            return;
        }

        spdlog::info("[modbus]Created RTU modbus with settings");
        spdlog::info("[modbus]Serial device path:{}", modbusConfigData.serialDevicePath);
        spdlog::info("[modbus]Serial baudrate:{}", modbusConfigData.modbusBaudrate);
        spdlog::info("[modbus]Modbus slave id:{}", modbusConfigData.modbusSlaveAddress);

        constexpr auto kSecondsTimeout{4};
        ...
        if (modbusConfigData.isDebugMode)
        {
        ...
        errCode = modbus_set_slave(m_pModbusHandle.get(), modbusConfigData.modbusSlaveAddress);
        if (isFailed(errCode))
        {
            spdlog::error(
        ...
        const std::vector<std::uint16_t>& registers)
    {
        ...
        spdlog::error(
            "[modbus] can't execute registers write. Error is:{}", modbus_strerror(errCode));
    }
...
ModbusRequestsProxy::ModbusRequestsProxy(const ModbusConfigData& applicationConfig)
    : m_pImpl{std::make_unique<ModbusRequestsProxyImpl>(applicationConfig)}
{
}
...
void ModbusRequestsProxy::scheduleRegistersWrite(
    std::uint16_t registerAddress,
    const std::vector<std::uint16_t>& registers)
{
    m_pImpl->scheduleRegistersWrite(registerAddress, registers);
}

std::unique_ptr<ModbusRequestsProxy> createModbusProxy(const ModbusConfigData& applicationConfig)
{
    return std::make_unique<ModbusRequestsProxy>(applicationConfig);
}